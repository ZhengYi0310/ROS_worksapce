...
class DisparityImage{
  ...
  public:
    DisparityImage(ros::NodeHandle& nh);
    ~DisparityImage();
};

DisparityImage::DisparityImage(ros::NodeHandle& nh){
  ImgCount = 0;
  // Set up the video writer
  // Define the Videowriter
  const std::string filename = "RGBwriter.avi";
  int fourcc = CV_FOURCC('M','P','4','V');
  cv::Size S = cv::Size(800, 600); 
  cv::VideoWriter outputVideo(filename, fourcc, 25, S);
  // Set up the window name
  std::string window_name = nh.resolveName("DisparityImage");

  ros::NodeHandle local_nh("~");
  bool autosize;
  local_nh.param("autosize",autosize, true);
  local_nh.param("window_name", window_name_, window_name);

  // Create a window for the disparity image
 
  //cvNamedWindow("DisparityImage", autosize ? CV_WINDOW_AUTOSIZE : 0);
  cvNamedWindow(window_name_.c_str(), CV_WINDOW_NORMAL);
  cvMoveWindow(window_name_.c_str(), 25, 625);
  ...
  g_signal_connect(widget, "destroy", G_CALLBACK(reboot), &sub_);
#endif

  std::string topic = nh.resolveName("/camera/depth/disparity");
  // Bring up the window
  cvStartWindowThread();
 
  // Subscribe to the topic
  topic = nh.resolveName("/camera/depth/disparity");
  sub_ = nh.subscribe<stereo_msgs::DisparityImage>(topic, 1,
&DisparityImage::disparityImageCallback, this); 
}  

void DisparityImage::disparityImageCallback(const stereo_msgs::DisparityImageConstPtr& msg){
  ...
  for (int row = 0; row < disparity_color_.rows; ++row){
    ...
    cv::Vec3b *disparity_color = disparity_color_[row], *disparity_color_end = disparity_color + disparity_color_.cols;
  ...
  //outputVideo.write(disparity_color_);
  sprintf(filename, "/home/yzheng/yizheng/image_view/DisparityImg%d.jpg", ImgCount);
...
int main(int argc, char **argv){
   ros::init(argc, argv, "DisparityImage", ros::init_options::AnonymousName);
   ros::NodeHandle nh;
   DisparityImage Image(nh);
