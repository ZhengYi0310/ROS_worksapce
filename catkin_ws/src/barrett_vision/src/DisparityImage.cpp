...
DisparityImage::DisparityImage(ros::NodeHandle& nh){
  ...
  cvNamedWindow("DisparityImage", CV_WINDOW_NORMAL=1);
  cvMoveWindow("DisparityImage", 25, 625);
  
  //#ifdef HAVE_GTK
  //g_signal_connect(GTK_WIDGET( cvGetWindowHandle("DisparityImage")), "destroy", G_CALLBACK(destroy), NULL);
  //#endif
  ...
  sub_ = nh.subscribe<stereo_msgs::DisparityImage>("/camera/depth/disparity", 1,
&DisparityImage::disparityImageCallback, this); 
}  

void DisparityImage::disparityImageCallback(const stereo_msgs::DisparityImage& msg){
